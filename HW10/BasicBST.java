...
public class BasicBST<Key extends Comparable<Key>, Value> {
    ...
    private int height(Node x, Key key) {
        if (x == null) return -1;
        int cmp = key.compareTo(x.key);
        if (cmp < 0) return height(x.left, key); // if the key is smaller than subtree's node.
        else if (cmp > 0) return height(x.right, key); // if the key is larger than subtree node.
        else return height(x);
    }
    ...
    public Iterable<Key> leaves() {
        ...
        Queue<Key> queue = new Queue<Key>();
        for (Key key : keys()) { // keys() from smallest to largest
            if (height(key) == 0) {
                queue.enqueue(key);
            }
        }
    ...
    public static void main(String[] args) {
        ...
        // StdOut.println("x height: " + st.height()); // S
        // StdOut.println("x.left height: " + st.height(st.root.left)); // E
        // StdOut.println("x.left.left height: " + st.height(st.root.left.left)); // A
        // StdOut.println("x.left.left.left height: " + st.height(st.root.left.left.left)); // null
        // StdOut.println("height of S: " + st.height("E"));
    }
}
